...
def ps_path(path: str | Path, home: Path) -> str:
    """Double-quoted PowerShell literal, using $HOME where possible.

    Forward slashes throughout: PowerShell and the Windows path APIs both take
    them, and it keeps the file identical in shape to picorc.
    """
    text = path.as_posix() if isinstance(path, Path) else str(path)
    home_posix = home.as_posix()
    prefix = ""
    if text == home_posix:
        ...
        text = text[len(home_posix) + 1 :]
        prefix = "$HOME/"
    # Backtick is PowerShell's escape character, not backslash.
    escaped = text.replace("`", "``").replace('"', '`"').replace("$", "`$")
    return '"' + prefix + escaped + '"'


def write_picorc_ps1(
    picorc_ps1: Path,
    home: Path,
    ...
    """PowerShell counterpart of picorc, for the default Windows shell."""
    lines = [
        ...
        "",
        "function picorc_PrependPath($Dir) {",
    ...
    for entry in reversed(path_entries):
        lines.append(f"picorc_PrependPath {ps_path(entry, home)}")
    lines += ["", "Remove-Item function:picorc_PrependPath", ""]
...
def include_from_rc(rc_file: Path, source_line: str) -> str:
    """Add (or refresh) the block holding source_line. Returns a status word."""
    block = "\n".join(
        [
            PICORC_BEGIN,
...
def main() -> int:
    ...
    parser.add_argument(
        "--no-rc-include",
        action="store_true",
        help="Write picorc but do not modify any shell config file",
    )
    parser.add_argument(
        "--github-path",
        action="store_true",
    ...
    if not args.no_picorc:
        ...
        if not args.no_rc_include:
            ...
            line = (
                f"if [ -f {shell_path(picorc, home)} ]; "
                f"then . {shell_path(picorc, home)}; fi"
            )
            status = include_from_rc(rc_file, line)
            print(f"Include block in {rc_file}: {status}")
            print(f"Run `source {picorc}` or open a new shell to pick up the tools.")

        # PowerShell is the default shell on Windows; picorc only helps the
        # Git Bash / MSYS2 side of it.
        if platform_key == "win32_x64":
            picorc_ps1 = picorc.with_name(picorc.name + ".ps1")
            write_picorc_ps1(
                picorc_ps1, home, path_entries, env_vars, sdk_version, platform_key
            )
