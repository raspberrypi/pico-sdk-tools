...
# directory, as those scripts do.  Any further patch directories to apply, such
# as the host specific packages/windows/patches, are given as arguments.
#
...
for patchesdir in "${BUILDDIR}/../packages/common/patches" "$@"; do
    [[ -d "${patchesdir}" ]] || continue
    # git -C runs from the tree being patched, so the patches need naming by an
    # absolute path however this was called.
    patchesdir=$(cd "${patchesdir}" && pwd)
    ...
    while IFS= read -r patchdir; do
        ...
        # Walk down to the tree being patched, checking out each submodule on
        # the way.  Nothing to check out for a top level repository: the build
        # scripts have cloned it already.
        srcdir="${BUILDDIR}"
    ...
    done < <(find "${patchesdir}" -type d 2> /dev/null | sort)
done
